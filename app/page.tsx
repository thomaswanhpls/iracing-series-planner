  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(233,69,96,0.12),transparent)]" />
        {/* Secondary blue glow bottom */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_50%_110%,rgba(15,52,96,0.2),transparent)]" />
        {/* Grid pattern */}
        <div
      ...
      <div className="relative z-10 flex flex-col items-center px-6 text-center">
        {/* Badge */}
        <div className="mb-8 animate-fade-in rounded-full border border-border-glow bg-bg-surface/80 px-4 py-1.5 font-display text-xs tracking-wider text-accent-primary backdrop-blur-sm">
          2026 SEASON 1 — BETA
        </div>

        {/* Title */}
        <h1 className="animate-slide-up font-display text-5xl font-bold leading-tight tracking-tight md:text-6xl lg:text-7xl">
          <span className="bg-gradient-to-b from-white to-text-secondary bg-clip-text text-transparent">
            iRacing
          </span>
          <br />
          <span className="bg-gradient-to-r from-accent-primary via-[#FF6B81] to-accent-primary bg-clip-text text-transparent">
            Season Planner
          </span>
        </h1>
        ...
        <div className="mt-24 grid max-w-4xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger-children">
          {features.map((feature) => (
            <div
              key={feature.title}
              className="group relative rounded-xl border border-border bg-bg-surface/50 p-6 text-left backdrop-blur-sm transition-all duration-300 hover:border-accent-primary/20 hover:bg-bg-surface/80 hover:shadow-[0_0_40px_rgba(233,69,96,0.06)]"
            >
              <div className="mb-4 inline-flex rounded-lg bg-accent-primary/10 p-2.5 text-accent-primary transition-colors group-hover:bg-accent-primary/15">
